...
namespace ropufu
{
    ...
    struct quiet_json
    {
        ...
        static bool try_parse(const nlohmann::json& entry, const std::string& name, std::array<t_value_type, t_size>& value) noexcept
        {
            ...
            if (count != t_size)
            {
                aftermath::quiet_error::instance().push(
                    aftermath::not_an_error::domain_error,
                    aftermath::severity_level::major,
                    std::string("JSON representation for ") + name + std::string(" has wrong size."), __FUNCTION__, __LINE__);
                return false;
            }
            count = 0;
            for (const nlohmann::json& sub_entry : entry)
            {
                value[count] = (sub_entry.get<t_value_type>());
                ++count;
            }
            return true;
        } // try_parse(...)
    ...
    public:
        explicit quiet_json(const std::string& caller_function_name, std::size_t caller_line_number) noexcept
        ...
        ~quiet_json() noexcept
        {
            if (this->m_has_parsed) return;
            aftermath::quiet_error::instance().push(
                aftermath::not_an_error::domain_error,
                aftermath::severity_level::major,
                std::string("JSON parsing failed."), this->m_caller_function_name, this->m_caller_line_number);
        } // ~quiet_json(...)

        void validate() noexcept { this->m_has_parsed = true; }
