    ...
    namespace sequential
    {
        namespace hypotheses
        {
            ...
            public:
                ...
                void run(process_type& proc, t_container_type& rules, t_on_start_type&& on_start, t_on_stop_type&& on_stop, std::size_t max_length = 1'000'000) noexcept
                {
                    ...
                    // ~~ Start ~~
                    on_start();
                    if (!aftermath::quiet_error::instance().good()) return; // Check for quiet errors.
                        ...
                        while (is_listening)
                        {
                            ...
                            if (t == max_length)
                            {
                                aftermath::quiet_error::instance().push(
                                    ...
                                    "Maximum run length exceeded.", __FUNCTION__, __LINE__);
                                return;
                            } // if (...)
                        } // while (...)
                        helper_type::toc(rules, proc); // Finalize another experiment run.
                        proc.reset();
                        if (!aftermath::quiet_error::instance().good()) return;
                    } // for (...)
